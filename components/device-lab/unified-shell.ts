...
const BASE_POLYGON_COUNT = 80_000;
...
function createUnifiedShellGeometry(gridResolution: number) {
  const maxPolygonCount = Math.max(
    30_000,
    Math.round(BASE_POLYGON_COUNT * (gridResolution / 72) ** 2),
  );
  ...
  const halfResolution = gridResolution / 2;
...
export function getUnifiedShellGeometry(
  gridResolution = DEFAULT_GRID_RESOLUTION,
) {
  const resolution = Math.max(32, Math.round(gridResolution));
  const cached = geometryCache.get(resolution);
  if (cached) return cached;

  const geometry = createUnifiedShellGeometry(resolution);
